...
export class StockpriceService {
  ...
  httpUrl='http://localhost:8080/stockprices/';
  ...
  deleteStockPrice(id:number):Observable<StockPrice>{
    return this.ht.delete(`http://localhost:8080/stockprices/${id}`);
  }
  updateStockPriceInfo(stockPrice :StockPrice):Observable<StockPrice>{
    return this.ht.put(`http://localhost:8080/stockprices` , stockPrice);
  }
  getStockPriceById(id :number):Observable<StockPrice>{
    return this.ht.get(`http://localhost:8080/stockprices/${id}`);
  }
}
